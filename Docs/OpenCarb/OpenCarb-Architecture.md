...
`OpenCarb` 的架构目标不是替代 `OpenClaw` 的 agent 执行能力，而是在其之上增加企业所需的控制面。系统需要同时满足以下要求：
...
flowchart TD
    ...
    sessionGateway --> controlPlane["OpenCarb Control Plane"]
...
- `OpenCarb Control Plane` 负责工作区、策略、模型、技能和审计治理。
...
`Control Plane` 是 `OpenCarb` 的核心差异化能力，由多个治理子系统组成。
...
V1 应坚持一个简单原则：`OpenCarb` 负责管控，`OpenClaw` 负责执行。只要控制面边界清晰，后续无论是更换模型、增加技能、扩展入口还是增强审批流，都可以在不推翻 runtime 的前提下渐进演进。
