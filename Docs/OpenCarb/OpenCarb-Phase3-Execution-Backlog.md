# OpenCarb Phase 3 执行 Backlog

## 1. 文档目的

本文为 Phase 3 规模化复制的可执行看板，对齐 [OpenCarb-Roadmap](OpenCarb-Roadmap.md) Phase 3 目标：标准部署模板、团队模板与默认策略、企业私有技能仓、跨团队能力复用。进入条件见 [OpenCarb-Phase2-Closeout-Review](OpenCarb-Phase2-Closeout-Review.md) 第 4 节。
...
- **部署模板**：`docker compose up -d --build` 可拉起 postgres、redis、control-plane、web-console；健康检查与就绪顺序见 [OpenCarb-Deployment-Template](OpenCarb-Deployment-Template.md)。
