# OpenCarb 与 OpenClaw Runtime 复用方案

## 1. 文档目的

本文说明 `OpenCarb` 如何复用 `OpenClaw` 的现有能力，并定义隔离适配层，避免企业版控制面与上游开源 runtime 发生强耦合。
...
以下内容不建议直接塞进 `OpenClaw` runtime 内部，而应由 `OpenCarb` 单独实现：
...
```mermaid
flowchart LR
    entryPoint["IDE/Web/PR入口"] --> accessGateway["OpenCarb Access Gateway"]
...
- 在 `OpenCarb` 接入层上封装 ACP 启动和 session 映射。
...
- `OpenCarb` 先根据工作区策略计算“批准后的可见技能视图”。
...
知识检索由 `OpenCarb Knowledge Service` 统一负责，不属于 `OpenClaw Runtime` 的原生权限域。
...
- `OpenClaw` 作为独立依赖版本管理。
- `OpenCarb` 对接的仅是有限接口：session、ACP、skill view、tool wrappers。
