...
flowchart TD
    ...
    ingress --> webConsole["OpenCrab Web Console"]
    ingress --> ideBridge["IDE/ACP Bridge"]
    webConsole --> controlPlane["OpenCrab Control Plane"]
...
- 检索权限和结果裁剪由 `OpenCrab Knowledge Service` 执行，runtime 不直接连接原始知识源。
...
`OpenCrab` 的安全重点不是把所有能力都关掉，而是把模型出口、技能可见性、工具权限和知识边界全部纳入统一控制面。只要部署上坚持“统一代理、默认内网、显式外发、完整审计”四个原则，就能在保证可用性的同时建立企业愿意接受的安全底座。
