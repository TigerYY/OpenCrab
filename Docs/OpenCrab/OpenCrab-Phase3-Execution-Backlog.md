...
| P3-E2-S4 | 从模板创建工作区时复制审批策略与 PR Review 配置 | 新工作区继承源工作区策略与配置，E2E 覆盖 | P3-E2-S2 |
| P3-E2-S5 | 模板详情返回策略/配置摘要（approvalPoliciesCount、prReviewConfigsCount） | 管理台可展示摘要 | P3-E2-S2 |
...
P3-E3-S1～S3 已验收：技能仓 API（GET registry/packages、GET registry/packages/:packageId/versions）、POST skills/packages 支持 sourceType=registry+sourceRef、管理台「技能仓」Tab 与「引入到工作区」闭环；phase3-smoke 与 E2E 已覆盖。

### P3-E4：跨团队能力复用

| ID | Story | DoD | 依赖 |
|----|-------|-----|------|
| P3-E4-S1 | 策略包导出/导入：审批策略可导出为 JSON，支持导入到新工作区（API + 管理台入口） | 已实现；契约与 E2E 一致 | P2 完成 |
| P3-E4-S2 | 多工作区共享技能包：同一技能仓版本可被多个工作区引用，权限与可见性规则明确 | 设计文档与最小实现 | P3-E3 |
| P3-E4-S3 | 运营手册：复制新团队的标准步骤、检查清单、回滚条件 | 见 OpenCrab-Phase3-Operations-Manual | P3-E2, P3-E3 |
...
- **模板 API**：`GET /api/workspace-templates`、`POST /api/workspace-templates`、`GET /api/workspace-templates/:id`（含策略摘要）、`POST /api/workspaces/from-template` 或 `POST /api/workspace-templates/:id/create-workspace` 可用；从模板创建时自动复制审批策略与 PR Review 配置；E2E 覆盖。
- **管理台**：Templates 页可查看模板列表、创建模板、从模板创建工作区（选择模板后可见策略摘要）、策略包导出/导入（Approval 页）。
- **Phase 3 冒烟**：`npm run smoke:phase3` 覆盖 workspace-templates、approval-policies/export、approval-policies/import、skills/registry/packages；CI 中与 phase2-smoke 同 job 执行。
- **技能仓**：GET /api/skills/registry/packages、GET /api/skills/registry/packages/:packageId/versions 可用；管理台「技能仓」页可浏览包与版本，并完成「引入到当前工作区」闭环；POST /api/skills/packages 支持 sourceType=registry、sourceRef=packageId@version；E2E 覆盖。
