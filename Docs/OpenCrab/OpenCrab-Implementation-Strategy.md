...
本文回答一个关键实施问题：`OpenCrab` 在后续产品开发中，应如何基于 `OpenClaw` 的已有成果和代码落地。重点是明确哪些能力适合直接复用，哪些能力必须自研，以及仓库和模块应如何拆分，才能兼顾落地速度、长期维护和上游兼容性。
...
- `OpenClaw` 作为执行内核复用。
- `OpenCrab` 独立仓库实现企业控制面。
- 通过适配层把工作区、权限、审计、模型路由和技能治理映射给 `OpenClaw Runtime`。

一句话概括：

`OpenCrab` 应采用“复用内核，外置控制面”的实施路线。
...
### 3.3 路线 C：独立构建 `OpenCrab`，复用 `OpenClaw` 执行内核
...
- 企业控制面全部由 `OpenCrab` 自己掌握。
- 不让 `OpenCrab` 的权限、审计和审批逻辑侵入到 `OpenClaw` 内部深处。
- 通过稳定的适配边界跟随上游升级。

### 4.2 核心判断

`OpenClaw` 解决的是“agent 如何执行”；`OpenCrab` 解决的是“企业里谁能执行、执行什么、怎么审计、何时审批、如何安全接入模型和知识”。
...
以下内容不建议塞进 `OpenClaw` 内部，而应由 `OpenCrab` 独立实现：
...
- `OpenClaw` 作为受控执行服务
- `OpenCrab` 全面负责模型、知识、策略、审计、审批和控制台
- 上游升级通过兼容矩阵和适配层回归来完成
...
- 所有权限、审计、审批、模型路由都放在 `OpenCrab` 控制面。
...
- 明确一条总规则：执行交给 `OpenClaw`，裁决交给 `OpenCrab`。

## 12. 最终建议

`OpenCrab` 的正确实施方式不是“从零重做”，也不是“长期 fork 后深度改造 `OpenClaw`”，而是：

1. 短期可基于 `OpenClaw` clone/fork 做技术验证。
2. 正式产品开发采用独立 `OpenCrab` 主仓。
3. `OpenClaw` 作为执行内核复用。
4. 企业能力全部外置到 `OpenCrab` 控制面。
